...
void error_norm(double rms[5])
{
  ...
  {
    #pragma dvm parallel([k][j][i] on u[k][j][i][]) reduction(sum(rms_loc)) \
                shadow_renew(u, rhs) private(m,xi,eta,zeta,add,u_exact)
...
void rhs_norm(double rms[5])
{
  ...
  #pragma dvm region targets(HOST)
  {
    #pragma dvm parallel([k][j][i] on u[k][j][i][]) reduction(sum(rms_loc)) private(m,add)
    for (k = 1; k <= nz2; k++) {
      for (j = 1; j <= ny2; j++) {
        for (i = 1; i <= nx2; i++) {
          for (m = 0; m < 5; m++) {
